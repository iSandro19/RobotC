...
task light() {
	...
	while(true) {
		...
		writeDebugStreamLine("Light = %d", luz);
		...
		if(bDoesTaskOwnSemaphore(sem_escape_light)) {
			...
			if(luz >= LIGHT_DETECTED) {
				writeDebugStreamLine("Light detected");
			...
			if(luz >= LIGHT_FOUND) {
				writeDebugStreamLine("Light found");
				//stopAllTasks();
				semaphoreUnlock(sem_light_follow);
			}

			if(luz <= LIGHT_LOST) {
				writeDebugStreamLine("Light lost");
				semaphoreUnlock(sem_light_follow);
		  }
			...
			if(currentDistance > MAX_DISTANCE && right_wall) {
				semaphoreLock(sem_follow_go);
					...
					repeatUntil(getGyroDegrees(gyroSensor) >= 5) {
						setMotorSpeed(leftMotor, DEFAULT_SPEED + 10);
